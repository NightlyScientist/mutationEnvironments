...
mutable struct Node
  ...
  nbors::UInt32
  y::Float32
  Node(n) = new(0, false, 0, 0, 0, 0, 0.0, n, 0)
end
...
function simulate!(graph, cntns, cli, dataModels, env)
  ...
  rates = SVector{4,Float64}([1 - sel, (1 - sel) * (1 + intensity), 1 - sel, (1 - sel) * (1 + intensity)])
  ...
  @inbounds while true
    ...
    node = graph[childIdx]
    node.y = reconstructCoordinatesReal(childIdx, width, height)[2]
    node.filled = true
    node.time = time
    node.ID_1 = groupID
